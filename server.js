...
/* ── Dynamic Early Bird Count from Bookings ─────────────────── */
app.get('/api/early-bird-count', async (req, res) => {
    try {
        const bookings = await fbRead('bookings');
        const count = bookings.reduce((sum, b) => {
            const type = b.ticket?.type || '';
            const cleanType = type.toLowerCase().replace('-', '');
            if (cleanType === 'earlybird') {
                return sum + (Number(b.ticket?.quantity) || 0);
            }
            return sum;
        }, 0);
        res.json({ count, max: 15 });
    } catch (err) {
        console.error('GET /api/early-bird-count error:', err.message);
        res.json({ count: 0, max: 15 });
    }
});
