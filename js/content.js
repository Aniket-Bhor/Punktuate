...
async function loadEvents() {
    ...
    renderEvents(window.globalEvents);
}
...
/** Find an event by id, _fbKey, or name slug */
function findEventBySlugOrId(eventId) {
    const events = window.globalEvents || [];
    if (!eventId) return null;
    // 1. Exact id / firebase key match
    let found = events.find(e => e.id === eventId || e._fbKey === eventId);
    if (found) return found;
    // 2. Full name slug match  (e.g. 'the-phoolish-concert')
    found = events.find(e => slugifyEvent(e.name) === eventId);
    if (found) return found;
    // 3. Partial / drop-the-word slug (e.g. 'phoolish-concert' matches 'the-phoolish-concert')
    found = events.find(e => {
        const s = slugifyEvent(e.name);
        return s.endsWith('-' + eventId) || s.startsWith(eventId + '-') || s === eventId;
    });
    if (found) return found;
    // 4. Any word from the slug appears in the event name slug
    const words = eventId.split('-').filter(w => w.length > 3);
    found = events.find(e => {
        const s = slugifyEvent(e.name);
...
/**
 * Open a specific event in the dynamic event page.
 * Navigates to event-dynamic first (which fetches the HTML),
 * then populates the DOM elements once the page is ready.
 */
function openDynamicEvent(eventId) {
    const evt = findEventBySlugOrId(eventId);
    if (!evt) {
        console.warn('[content] Event not found:', eventId);
        return;
    }

    // Store the event so _populateDynamicEvent can use it after navigation
    openDynamicEvent._pending = evt;

    // Use clean name-based slug in URL for shareability
    const urlSlug = slugifyEvent(evt.name) || evt.id || eventId;
    router.navigate('event-dynamic?id=' + urlSlug);

    // Populate once the DOM is ready (router uses requestAnimationFrame for fade-in)
    requestAnimationFrame(() => {
        requestAnimationFrame(() => _populateDynamicEvent(evt));
    });
}

/** Fill the event-dynamic page elements with event data */
function _populateDynamicEvent(evt) {
    ...
    if (dateEl) {
        dateEl.innerHTML = (evt.date && evt.time)
            ? `${evt.date} @ ${formatTime12hr(evt.time)}`
            : (evt.date || 'TBA');
    }
...
// Events are PRE-FETCHED into globalEvents so openDynamicEvent() works
// immediately without waiting — but we do NOT call renderEvents() here
// because #events-grid doesn't exist until the user navigates to events-hub.
// Rendering is triggered by router._initPage('events-hub') in js/router.js.
document.addEventListener('DOMContentLoaded', () => {
    loadAnnouncements();
    // Pre-fetch only: populate globalEvents without rendering
    _prefetchEvents();
});

async function _prefetchEvents() {
    const events = await fetchCollection('events', getDefaultEvents);
    window.globalEvents = events;
    // No renderEvents() call here — grid doesn't exist yet
}
